...
function BusinessDrawer({ publicId, onClose }) {
  ...
  return <div className="drawer-backdrop"><aside className="drawer"><header><div><h2>{b.businessName || 'Business'}</h2><p>{b.ownerName || ''} · {b.city || ''}</p></div><button className="btn ghost" onClick={onClose}>Close</button></header>
    {loading ? <Loader /> : error ? <Alert type="error">{error}</Alert> : <>
      <div className="tabs">{['overview','edit','users','subscription','whatsapp','sales','inventory','customers','suppliers','purchases','expenses','cashbook','staff','support','audit','billing','performance'].map((t) => <button key={t} onClick={() => setTab(t)} className={tab === t ? 'active' : ''}>{t}</button>)}</div>
      ...
      {tab === 'suppliers' && <BusinessDataTab loader={() => AdminApi.businessSuppliers(publicId)} type="suppliers" />}
      {tab === 'purchases' && <BusinessDataTab loader={() => AdminApi.businessPurchases(publicId)} type="purchases" exportable />}
      {tab === 'expenses' && <BusinessDataTab loader={() => AdminApi.businessExpenses(publicId)} type="expenses" exportable />}
      {tab === 'cashbook' && <BusinessDataTab loader={() => AdminApi.businessCashbook(publicId)} type="cashbook" exportable />}
      {tab === 'staff' && <BusinessDataTab loader={() => AdminApi.businessStaff(publicId)} type="staff" />}
      {tab === 'support' && <BusinessDataTab loader={() => AdminApi.businessSupportTickets(publicId)} type="support" />}
      {tab === 'audit' && <BusinessDataTab loader={() => AdminApi.businessAuditLogs(publicId)} type="audit" exportable />}
...
function Tools(){return <section className="card"><CardHead title="Useful admin tools" sub="Recommended controls for your SaaS"/><div className="tool-grid"><div><b>Audit logs</b><span>Track edits, blocks, approvals and login activity.</span></div><div><b>Expired auto-block</b><span>Later add a daily job to block expired businesses automatically.</span></div><div><b>Backup exports</b><span>Export customers, suppliers, sales, expenses, cashbook and inventory per business.</span></div><div><b>Support access</b><span>Create support users without full delete access.</span></div><div><b>Business data view</b><span>Open a business to view suppliers, purchases, expenses, cashbook, staff, support and audit logs.</span></div><div><b>WhatsApp templates</b><span>Manage invoice and payment reminder templates.</span></div></div></section>}
